...
This repository contains `TreeSnapshots`, a powerful script that automates the process of recording file system structures on **macOS** or **Linux**. It uses Git to efficiently track changes over time.
...
-   **🤖 OS Agnostic**: Works seamlessly on macOS and major Linux distributions.
-   **💾 Interactive Drive Selection**: Automatically lists mounted drives and lets you choose which one to scan.
...
> **Note**: On Linux, the `tree` command is required. If not installed, the script will attempt to install it for you.
